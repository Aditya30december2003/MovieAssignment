...
const MangaInfo = () => {
  ...
  const getData = async()=>{
    try {
      const response = await fetch(`https://api.jikan.moe/v4/manga/${mal_id}/full`)
  ...
  return (
    ...
    <div className="flex bg-black text-white p-7 w-[95%] h-screen gap-4 ml-[5%]">
     {/*  */}
      <div className="w-[30%]">
        <img
          className="h-[30rem] w-full object-cover rounded-md border-white border-2"
      ...
      <div className="w-[60%]">
        ...
        <p className='text-white/60'>Chapters:</p><p>{item?.chapters}</p>
        <p className='text-white/60'>Volumes:</p><p className=''>{item?.volumes}</p>
        ...
        {item?.type==="TV" && "ONA" && "OVA" && "Special" ?<div className='px-4 flex items-center mt-5 ml-1 cursor-pointer rounded-md font-bold text-black bg-white justify-between w-[45%]'>
         <CgScreen size={40}/>
         <p className='text-[1.3rem]'>Volumes</p>
     ...
     {/* <Row type='manga' title='You may also like' prop="recommendations/manga" rowID='1'/> */}
     </>
  );
};
